...
public interface AppointmentRepository extends JpaRepository<Appointment, Integer> {
    ...
    List<Appointment> findAppointmentsByVetOrderBySlotAsc(User vet);
    ...
    @Query("""
        ...
        FROM Appointment appointment
        WHERE (appointment.pet is not null AND appointment.pet.owner = :owner AND appointment.status = 'BOOKED' AND appointment.slot >= :currentDate)
        ORDER BY appointment.slot
        LIMIT :k
        """)
    List<Appointment> findTopKByOwnerOrderBySlotAsc(User owner, Integer k, LocalDateTime currentDate);
    @Query("""
        SELECT appointment
        FROM Appointment appointment
        WHERE (appointment.vet is not null AND appointment.vet = :vet AND appointment.status = 'BOOKED' AND appointment.slot >= :currentDate)
        ...
        LIMIT :k
        """)
    List<Appointment> findTopKByVetOrderBySlotAsc(User vet, Integer k, LocalDateTime currentDate);
}
